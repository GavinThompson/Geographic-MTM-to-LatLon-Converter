class MTMConverter {
    ...
    // adapted from NSRUG  (caution: longitudes are +ve here)
     gsrugZoner(mtmLat, mtmLong, mtmZone) {
    ...
    convert( xCoord, yCoord){
      ...
      const ellipsoid = this.geo_constants(ellipsoidName);
      ...
      let longorig = (this.zone - 1) * 6 - 180 + 3;  //+3 puts origin in middle of zone
      ...
      if (scaleTm == 0.9999)
      {
          ...
          if (meridian == 0)
          {
             let zonerResult = this.gsrugZoner(0., 0., this.zone);
      ...
      const phi1Rad = mu + (3 * e1 / 2 - 27 * e1 * e1 * e1 / 32) * this.sin(2 * mu) + (21 * e1 * e1 / 16 - 55 * e1 * e1 * e1 * e1 / 32) * this.sin(4 * mu) + (151 * e1 * e1 * e1 / 96) * this.sin(6 * mu);
      const phi1 = phi1Rad * this.rad2deg;
      ...
      let lat = phi1Rad - (N1 * this.tan(phi1Rad) / R1) * (D * D / 2 - (5 + 3 * T1 + 10 * C1 - 4 * C1 * C1 - 9 * eccPrimeSquared) * D * D * D * D / 24 + (61 + 90 * T1 + 298 * C1 + 45 * T1 * T1 - 252 * eccPrimeSquared - 3 * C1 * C1) * D * D * D * D * D * D / 720);
      lat = lat * this.rad2deg;
      
      let lon = (D - (1 + 2 * T1 + C1) * D * D * D / 6 + (5 - 2 * C1 + 28 * T1 - 3 * C1 * C1 + 8 * eccPrimeSquared + 24 * T1 * T1) * D * D * D * D * D / 120) / this.cos(phi1Rad);
